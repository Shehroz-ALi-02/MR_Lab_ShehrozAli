...
The workflow difference is significant: SLAM is an exploratory process where the robot discovers the environment and builds a coordinate system from scratch. Navigation is a "knowledge-based" process. The robot uses a fixed reference (the map) to solve the "Where am I?" problem (Localization) and the "How do I get there?" problem (Path Planning). While SLAM focuses on discovery, Navigation focuses on precision and reliability.
